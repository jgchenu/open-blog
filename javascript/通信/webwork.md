...
```
运行程序可以从 console 看到，main.js 中接收到来自 worker.js 中的一个运行错误，在监听事件的函数中接受一个参数 `event` 这个事件对象中有几个比较重要的参数需要我们注意：
...
```
上面是一段嵌入网页的脚本，注意必须指定`<script>`标签的`type`属性是一个浏览器不认识的值，上例是app/worker。
...
#### 而且我在做这个demo的实现，发现只是一个普通的worker，并没有实现share通信。

### 九.兼容性

最后上一下这个Worker 的兼容性
  ![webworker兼容](https://raw.githubusercontent.com/jgchenu/staticAssets/master/web-learning/webworker.png)