    ...
    return async (ctx, next) => {
        await new Promise((resolve, reject) => {
            ...
            ctx.req.on("end", () => {
                ...
                // 获取不同系统的换行符
                let lineBreak = process.platform === "darwin" ? "\r\n" : "\n";//mac 下 platform为darwin为"\r\n"， wind32 为"\r\n"，win64 为"\r"
                
                // 非文件类型数据的最终返回结果
                let fields = {};
                // 分隔的 buffer 去掉没用的头和尾即开头的 '' 和末尾的 '--'
                dataArr = Buffer.concat(dataArr).split(bondery).slice(1, -1);
                ...
                dataArr.forEach(lines => {
                    ...
                    if (head.includes("filename")) {
                        ...
                        let extendtion=head.toString().match(/filename="(.*)"/)[1].split('.')[1];
